...
export default function App() {
  ...
  useEffect(() => {
    const cleanup = addMessageListener((message) => {
      ...
      if (type === 'YOUTUBE_SUBTITLES_LOADED' && ytSubtitles && videoId === currentVideoId) {
        const filteredSubtitles = ytSubtitles.filter((sub: any) => isValidSubtitleText(sub.text));

        setTracks(prev => {
          const newTracks = prev.length === 0 
            ? [{ id: 'intercepted', language: '中文 (简体)', subtitles: filteredSubtitles, isAutoGenerated: true }]
            : prev.map(t => {
  ...
  useEffect(() => {
    ...
    const activeSubtitle = currentTrack.subtitles.find(
      (sub: any) => currentTime >= sub.start && currentTime < sub.start + sub.duration && isValidSubtitleText(sub.text)
    );
