...
function getYoutubeCaptions() {
  ...
  try {
    // Try to get from the player element first (most reliable if initialized)
    const player = document.getElementById('movie_player') as any;
    if (player && player.getOptions) {
      const captionTracks = player.getOptions('captions')?.captionTracks;
      if (captionTracks && captionTracks.length > 0) {
        sendTracks(videoId, captionTracks);
        return;
      }
    }

    // Fallback: Look for ytInitialPlayerResponse in script tags
    const scripts = Array.from(document.getElementsByTagName('script'));
    for (const script of scripts) {
      const content = script.innerText;
      if (content.includes('ytInitialPlayerResponse')) {
        const match = content.match(/ytInitialPlayerResponse\s*=\s*({.+?});/) || 
                      content.match(/ytInitialPlayerResponse\s*=\s*({.+?})\s*;/);
...
function sendTracks(videoId: string, captionTracks: any[]) {
  sendMessage({
    ...
    tracks: captionTracks.map((t: any) => ({
      ...
      baseUrl: t.baseUrl,
      isAutoGenerated: (t.vssId || t.vss_id || '').startsWith('a.') || t.kind === 'asr'
    }))
  });
}
...
function createUI() {
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL) {
    iframeSrc = chrome.runtime.getURL('index.html');
  } else {
    iframeSrc = window.location.origin + '/index.html';
  }
  ...
  const urlParams = new URLSearchParams(window.location.search);
  container.style.display = urlParams.get('v') ? 'block' : 'none';
...
const observer = new MutationObserver(() => {
  ...
  if (container) {
    container.style.display = videoId ? 'block' : 'none';
  }
...
window.addEventListener('message', (event) => {
  if (event.data.type === 'PLAYER_RESPONSE_FOUND') {
    const data = event.data.data;
    const captionTracks = data?.captions?.playerCaptionsTracklistRenderer?.captionTracks || [];
    const vid = currentVideoId || new URLSearchParams(window.location.search).get('v');
    if (captionTracks.length > 0 && vid) {
      sendTracks(vid, captionTracks);
    }
  }
  if (event.data.type === 'TIMEDTEXT_INTERCEPTED') {
    ...
    try {
      ...
      videoId = urlObj.searchParams.get('v') || currentVideoId;
    } catch (e) {
